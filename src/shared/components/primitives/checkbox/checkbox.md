...
const AceptoTerminos = () => {
  ...
  return <Checkbox checked={acepto} onChange={setAcepto} />;
};
```
...
const VerificacionRobot = () => {
  ...
  return <CheckboxRobot checked={verificado} onChange={setVerificado} />;
};
```
...
const FilaAcepto = ({
  ...
  <label className="flex cursor-pointer items-center gap-2" onClick={() => onChange(!checked)}>
